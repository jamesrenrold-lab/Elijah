...
- **Octonary Active Power — Drowned Domain:** pulls the hostile mob you are directly looking at (up to 40 blocks away) into a separate, always-day dimension for **40 seconds**. The transfer and cooldown lifecycle were rebuilt independently: Forge's standard `ITeleporter` path moves the real mob between dimensions, with none of the Echo clone/discard/NBT reconstruction code. A single authoritative server session is removed before cleanup begins, and a visible **5-second cooldown starts only when the domain closes**, so a stale session cannot permanently lock the key. Connector's temporary Origin-power removal during dimension changes is ignored. The caster begins submerged inside a two-block-deep lagoon while the target starts deep on the raised sandy crescent at X=-34; the crescent now touches the domain edges, and a nine-block-wide sandstone stair ramp connects the water to the beach. The dimension's infinite 60-deep water-over-sand generator was replaced by solid sandstone topped with exactly two water blocks. Existing central chunks receive the same opaque sandstone seabed once, and every sand island has a sandstone shelf, eliminating falling-sand physics and deep-water rendering. Four flagship-scale three-mast galleons remain broadside-on beyond the arena walls. Their visible inward broadsides use seven CBC cannons each; the fifty-two unseen/redundant cannons and their ticking block entities from the old fleet are removed during migration. After five seconds, cannonballs alternate between the four walls every half-second and scatter around the target. They detonate on collision or traced impact, outline a four-block blast, and deal **55% of the caster's current attack damage + 1.5 damage per completed 10% Curse**. The caster gains **+50% movement speed**, Dolphin's Grace III, Regeneration II, and **+50% lifesteal**. Arena generation is versioned and runs once; orphaned cannonballs are purged between sessions, while pathfinding, effect refresh, projectile synchronization, tracers, warning marks, and blast particles are all throttled.
