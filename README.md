...
- **Octonary Active Power — Drowned Domain:** pulls the hostile mob you are directly looking at (up to 40 blocks away) into a separate, always-day dimension for **40 seconds**. The transferred mob is made persistent, permanently targets the caster, and is actively navigated toward them for the entire session. Connector's temporary Origin-power removal during dimension changes is explicitly ignored, preventing it from firing the normal Origin-loss cleanup and instantly closing the domain. A brief level-transition mismatch is also tolerated. The working Echo transfer order removes the source before registering its same-UUID destination copy. Every session retains a complete pre-domain NBT snapshot, so an unexpectedly missing target is reconstructed at its exact original dimension and position rather than being lost. The caster begins inside a large two-block-deep lagoon while the target starts deep on the raised sandy crescent at X=-34; a nine-block-wide, three-tier sandstone stair ramp connects the lagoon to the beach. The beach has layered dunes and a curved, eight-frond palm; twelve additional dune islands ring the far ocean outside the barrier. Four flagship-scale three-mast galleons sit broadside-on beyond the arena walls. Each has a deep seven-layer tapered hull, raised forecastle, two-storey sterncastle, separate upper/lower **three-block-billowed sails**, rigging, cabin windows, a gilded prow, and **ten sideways CBC cannons per broadside** firing through an enclosed lower gun deck when Create Big Cannons is installed. The arena is built once at server startup and marked persistently instead of rewriting tens of thousands of blocks on every cast. Target pathfinding runs twice per second, buffs refresh once per second, and the exact-radius cannon smoke uses roughly one-fifth of its former particle packets. After five seconds, visible server-traced cannonballs alternate between the four ship-facing walls every half-second and scatter around the target. They detonate only when they touch a hitbox or traced impact, outline their exact four-block blast volume with a short-lived smoke shell, and deal **55% of the caster's current attack damage + 1.5 damage per completed 10% Curse**. The caster gains **+50% movement speed**, Dolphin's Grace III, Regeneration II, and **+50% lifesteal**. The testing build has **no cooldown at all**. Expired or stale sessions are recovered automatically even when the tick loop encountered an integration exception; only a genuinely active 40-second session prevents overlap and reports its remaining time.
