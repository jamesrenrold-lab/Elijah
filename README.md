...
- **Octonary Active Power — Drowned Domain:** pulls the nearest hostile mob within 40 blocks into a completely separate, always-day island dimension for **40 seconds**. The arena is an expanded half-sand/half-water island with dunes, a palm tree, an ocean surrounding it, four large multi-deck pirate galleons outside the barrier, and the same forced-return behavior as the existing domain. The island has an invisible barrier floor and walls; the water side is exactly two sealed source blocks deep. You gain **+50% movement speed**, Dolphin's Grace III for fast swimming, Regeneration II, and **+50% lifesteal**. Five seconds after entry, quiet black cannonball shots begin dropping from above at a random point 4–10 blocks around the target and strike every half-second. Each shot uses a controlled 4-block radial blast with a short dark-dust/smoke puff and no block destruction. If Create Big Cannons is installed, the four ships use its real carriage, mount, cast-iron chamber, barrel and end blocks; the mod remains optional. The domain uses the free `key.origins.octonary_active` binding and has a five-second cooldown for testing.
