...
- **Octonary Active Power — Drowned Domain:** pulls the hostile mob you are directly looking at (up to 40 blocks away) into a separate, always-day dimension for **40 seconds**. Activation is deferred to the next server tick so Connector cannot replace the Origins cooldown power halfway through its callback—the cause of the former once-per-life lock. Forge's standard `ITeleporter` path moves the real mob between dimensions, with none of the Echo clone/discard/NBT reconstruction code. A visible **5-second cooldown starts only when the domain closes**. The caster begins submerged inside a two-block-deep lagoon while the target starts at X=-34 on a complete raised circular sand arena with matching east and west ramps. A thick circular barrier follows the outside of the sand instead of exposing square corners. Solid sandstone beneath two water blocks prevents falling-sand physics and deep-water rendering. Four flagship-scale three-mast galleons remain broadside-on beyond the arena wall, with seven visible inward CBC cannons each. After five seconds, cannonballs alternate between the four walls every half-second. Each shell snapshots the target's exact hitbox center when fired and follows that fixed line—never homing—so moving immediately dodges the recorded impact point. The tell and smoke outline mark a **4.5-block blast**, dealing **55% of the caster's current attack damage + 1.5 damage per completed 10% Curse**. The caster gains **+50% movement speed**, Dolphin's Grace III, Regeneration II, and **+50% lifesteal**.
