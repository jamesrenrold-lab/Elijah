...
- **Blood Rush:** each activation adds **20** charge immediately and starts a **10-second active window**. During that window the meter rises by **1** per second; while inactive it drains by **1** every **4 seconds** (0.25 per second). At 100 charge it automatically consumes the meter and triggers a **25-second overfill** with Blindness, Darkness, Strength, **50% lifesteal**, and health degeneration that is clamped so it cannot kill the player.
...
- **Octonary Active Power — Drowned Domain:** pulls the nearest hostile mob within 40 blocks into a completely separate, always-day island dimension for **40 seconds**. The arena is an expanded half-sand/half-water island with dunes, a palm tree, an ocean surrounding it, four pirate ships outside the barrier, and the same forced-return behavior as the existing domain. You gain **+50% movement speed**, Dolphin's Grace III for fast swimming, Regeneration II, and **+50% lifesteal**. Five seconds after entry, black cannonball shots begin appearing within five blocks of the target and strike every half-second. The domain uses the free `key.origins.octonary_active` binding and has the existing four-minute cooldown.
