...
- **Blood Rush:** each activation adds **20** charge immediately and starts a **10-second active window**. During that window the meter rises by **1** per second; while inactive it drains by **1** every **4 seconds** (0.25 per second). At 100 charge it automatically consumes the meter and triggers the existing 20-second overdrive effects.
