...
- **Octonary Active Power — Drowned Domain:** pulls the directly-looked-at hostile mob within 40 blocks into a separate, always-day island dimension for **40 seconds**. The enlarged arena keeps the water exactly two source blocks deep but gives it more than half of the playable surface, with the sand, dunes and palm tree on the remaining side. Four larger three-masted galleons sit beyond the walls with six Create Big Cannons placements each when CBC is installed. Five seconds after entry, fast cannonballs alternate between the four walls as if fired by those ships, with a muzzle flash and compact 1.25-block warning ring. A projectile explodes only after touching an entity or block; its controlled blast has a **4-block radius**, light dust/smoke, modest knockback and no block destruction. Cannonball damage is **30% of the caster's current attack damage + 1.5 damage per completed 10 Blood charge**. The caster gains **+50% movement speed**, Dolphin's Grace III, Regeneration II and **+50% lifesteal**. The domain uses `key.origins.octonary_active`; its **five-second cooldown now belongs to the Origins power and renders on its HUD bar**, with no separate hidden Java cooldown.
